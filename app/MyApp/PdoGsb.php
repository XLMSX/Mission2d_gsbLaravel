...
class PdoGsb
{
	...
	public function getInfoVisisteurById($idVisiteur)
	{
	...
	public function getLesFichesFraisARembourser()
	{
		$req = "SELECT idEtat, dateModif, nom, prenom, visiteur.id as idVisiteur FROM fichefrais 
		INNER JOIN visiteur ON fichefrais.idVisiteur = visiteur.id WHERE idEtat = 'VA'";
		$rs = $this->monPdo->query($req);
		$ligne = $rs->fetchAll();
		return $ligne;
	}

	public function updateFicheFraisRembourser($idVisiteur)
	{
		$dateActuelle = date("Y-m-d");
		$req = "UPDATE fichefrais SET idEtat = 'RB', dateModif = '$dateActuelle' WHERE idVisiteur = :idVisiteur AND idEtat = 'VA'";
		$rs = $this->monPdo->prepare($req);
		$rs->bindParam(':idVisiteur', $idVisiteur);
		$rs->execute();
	}

	public function updateFicheFraisMiseEnPaiment($idVisiteur)
	{
		$date = date("m") === 1 ? 12 : date("m") - 1;
		$dateActuelle = date("y-m-d");
		$req = "SELECT * FROM fichefrais AS fi
				INNER JOIN visiteur AS vi ON fi.idVisiteur = vi.id 
				WHERE datemodif =
					(UPDATE fichefrais AS fi SET dateModif = $dateActuelle 
				 	WHERE dateModif = $date) ";
	}
